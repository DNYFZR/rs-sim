...
pub fn engine(output_dir:&str, n_sims:i64, n_steps:i64, states: Vec<i64>, probabilities:Vec<f64>, target_value:i64, target_mapping: Option<Vec<i64>>, limit_array:Option<Vec<i64>>) {
    ...
    for batch in 0..loop_batches {
        ...
        (0..batch_size).into_par_iter().map(|sim_id| {            
            ...
            execute_event(output_dir, run_id, states.clone(), probabilities.clone(), n_steps, target_mapping.clone(), limit_array.clone())
    ...
    // Aggregations
    tx::aggregate(output_dir, n_sims as i64, target_value, target_mapping.clone(), false).expect("failed to complete aggregation...");
    tx::aggregate(output_dir, n_sims as i64, target_value, target_mapping.clone(), true).expect("failed to complete constrained aggregation...");
...
fn execute_event(output_dir:&str, run_id:i64, states:Vec<i64>, probabilities:Vec<f64>, n_steps:i64, target_mapping: Option<Vec<i64>>, limit_array:Option<Vec<i64>>) -> Result<(), PolarsError> {
    let mut tmp_df = &mut tx::to_df(&discrete_event(states.clone(), probabilities.clone(), n_steps));
            
    if target_mapping.is_some() {
        tmp_df = tmp_df.with_column(Series::from_vec(PlSmallStr::from_str("cost"), target_mapping.clone().unwrap()) )?;
    }
    ...
    // Write to storage
    pq::write(tmp_df, &format!("{}/result_{}.parquet", &output_dir, &run_id))?;
